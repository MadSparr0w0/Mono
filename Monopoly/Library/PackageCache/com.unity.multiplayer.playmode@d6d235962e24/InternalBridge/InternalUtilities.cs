...
namespace Unity.Multiplayer.PlayMode.Editor.Bridge
{
    internal class InternalUtilities
    {
        ...
        internal static bool IsBuildProfileSupported(BuildProfile buildProfile)
        {
            ...
            var isBuildTargetSupported = BuildPipeline.IsBuildTargetSupported(buildTargetGroup, buildTarget);

            if (buildSubtarget == StandaloneBuildSubtarget.Server)
            {
                return isBuildTargetSupported && ServerPlayerInstalled(buildTarget);
            }
            return isBuildTargetSupported;
        }

        private static bool ServerPlayerInstalled(BuildTarget buildTarget)
        {
            var playerPackage = BuildPipeline.GetPlaybackEngineDirectory(buildTarget, BuildOptions.None, false);
            string variation;
            ...
            switch (buildTarget)
            {
                case BuildTarget.StandaloneWindows or BuildTarget.StandaloneWindows64:
                    variation = "win64_server_development_mono";
                    relativePathToExecutable = "UnityPlayer.dll";
                    break;
                case BuildTarget.StandaloneOSX:
                    variation = "macos_x64_server_development_mono";
                    relativePathToExecutable = "UnityPlayer/UnityPlayer";
                    break;
                case BuildTarget.StandaloneLinux64:
                    variation = "linux64_server_development_mono";
                    relativePathToExecutable = "LinuxPlayer";
                    break;
                default:
                    return true;
            }

            var path = new List<string> { playerPackage, "Variations", variation };
            path.Add(relativePathToExecutable);
            return File.Exists(Path.Combine(path.ToArray()));
        }
